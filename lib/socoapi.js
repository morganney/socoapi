 ...
 *
 * @param {String:port}
 * @param {Number:cachettl} milliseconds to store cached values for before deleting. (false or null disable caching)
 * @param {Function:callback} optional callback.
 */
socoapi.listen = function(port, cachettl, callback) {
  var ttl = parseInt(cachettl);

  port = port || '3535';

  if(!ttl) DRY.c.caching = false; // opt out of caching.
  else if(!isNaN(ttl)) DRY.c.cachettl = ttl;

  proxy.listen(port, function() {
    if(typeof callback === 'function') callback(port, ttl);
  });
};
